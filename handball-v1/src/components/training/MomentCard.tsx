...
import {
  ...
  ArrowUp, ArrowDown, Upload, X, Plus, Settings,
} from 'lucide-react'
    ...
    <>
      <div
        ...
        className={clsx(
          ...
          isDragging
            ? 'shadow-2xl border-gold-400 z-50 opacity-95'
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-50">
          ...
          <div className="flex-1 min-w-0">
            ...
            {collapsed && moment.description && (
              <p className="text-xs text-gray-400 truncate">{moment.description}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-0.5">
            <button
              disabled={index === 0}
              onClick={onMoveUp}
              className="p-1 text-gray-300 hover:text-dj-600 disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <ArrowUp size={15}/>
            </button>
            <button
        ...
        {!collapsed && (
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Lista desplegable de ejercicio (editable) */}
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700">Nombre del ejercicio</label>
                  <button
                    onClick={() => setShowLabelMgr(true)}
                    className="text-xs text-dj-600 hover:text-dj-800 flex items-center gap-1"
                  >
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 mb-1.5">
                  Imagen <span className="text-gray-400 font-normal">(captura TacticalBoard)</span>
                </p>
                {moment.image_url ? (
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="w-full h-28 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-1.5 text-gray-400 hover:border-dj-400 hover:text-dj-600 transition-colors"
                  >
                    {uploading ? (
                      ...
                      <>
                        <Upload size={20}/>
                        <p className="text-xs font-medium">Subir captura</p>
                        <p className="text-xs text-gray-300">PNG, JPG hasta 5 MB</p>
                      </>
                    )}
                  </button>
                )}
      ...
      {/* ── Modal: administrar lista de ejercicios ──────────────────── */}
      <Modal
        open={showLabelMgr}
        onClose={() => setShowLabelMgr(false)}
        title="Editar lista de ejercicios"
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" onClick={handleAddLabel} icon={<Plus size={14}/>}>
              Agregar
            </Button>
          </div>
          ...
          <p className="text-xs text-gray-400">
            Los cambios se aplican a todos los entrenamientos.
          </p>
        </div>
      </Modal>
