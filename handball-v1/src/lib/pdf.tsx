...
import type { TrainingSession, Moment, Account } from '@/types'
...
function MomentoBlock({ moment, index, coachName, s }: {
  moment: Moment; index: number; coachName: string; s: ReturnType<typeof buildStyles>
}) {
  return (
...
function TrainingDocument({ session, account }: { session: TrainingSession; account?: Account | null }) {
  ...
  return (
    <Document title={`Entrenamiento ${session.team_category} - Sesión ${session.session_number}`}>
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        <View style={s.grid}>
          {sorted.map((m, i) => (
            <MomentoBlock key={m.id} moment={m} index={i} coachName={session.coach_name} s={s}/>
          ))}
        </View>
      </Page>
    </Document>
...
export async function downloadTrainingPDF(session: TrainingSession, account?: Account | null) {
  const blob = await pdf(<TrainingDocument session={session} account={account}/>).toBlob()
...
export async function getPDFBlob(session: TrainingSession, account?: Account | null): Promise<Blob> {
  return pdf(<TrainingDocument session={session} account={account}/>).toBlob()
}
