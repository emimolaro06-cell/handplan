...
import { ChevronLeft, ChevronRight, Save, FileDown, Plus, X, ChevronDown, ChevronUp } from 'lucide-react'
...
export function MonthlyPlanPage() {
  ...
  const [showInfo, setShowInfo] = useState(false) // panel info colapsable
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-4">
          ...
          {/* Navegación de mes inline */}
          <div className="flex items-center gap-2 bg-dj-800 rounded-xl px-3 py-2">
            <button onClick={() => setCurrentDate(d => new Date(d.getFullYear(), d.getMonth()-1, 1))} className="text-white/60 hover:text-white">
              <ChevronLeft size={18}/>
            </button>
        ...
        <div className="flex gap-2">
          <Button variant="secondary" size="sm" icon={<FileDown size={15}/>} onClick={() => plan && downloadMonthlyPDF(plan as Record<string,unknown>, currentDate)}>
            PDF
          </Button>
              ...
              >
                ...
                <div className="space-y-1">
                  {dayData.contents.map(c => (
                    <div key={c} className={clsx('text-xs px-2 py-1 rounded-lg font-bold truncate', CONTENT_COLOR[c])}>
                      {CONTENT_SHORT[c]}
                    </div>
                  ))}
                  {dayData.note && (
                    <div className="text-xs px-2 py-1 rounded-lg bg-gold-100 text-gold-800 font-medium truncate">
                      {dayData.note}
                    </div>
                  )}
